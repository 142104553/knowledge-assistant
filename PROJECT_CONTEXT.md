...
构建基于 RAG + LangChain + Agent 的私有领域知识库问答系统，面向电力行业技术文档（调度、保护、配电、设备运维）提供智能检索与专业问答服务。
...
│   ├── retrievers/hybrid.py # HybridRetriever (Dense only, BM25 待接入)
│   └── post_processors/reranker.py
├── ingestion/
...
├── vectorstore/
│   └── factory.py           # Chroma 实现 (delete_by_doc_id)
├── models/
...
2. **BM25 未启用**：`HybridRetriever` 仅 Dense 检索生效
